...
inline int alt_greyscale(Mat& src, Mat& dst) {
...
inline int blur5x5(Mat& src, Mat& dst) {
    Mat tmp;
    src.copyTo(tmp);
    for (int i = 0; i < src.rows; i++) {
...
inline int sobelX3x3(Mat& src, Mat3s& dst) {
    Mat3s tmp;
    src.copyTo(tmp);
...
inline int sobelY3x3(Mat& src, Mat3s& dst) {
    Mat3s tmp;
    src.copyTo(tmp);
    //[1 2 1]
    for (int i = 0; i < src.rows; i++) {
...
inline int magnitude(Mat3s& sx, Mat3s& sy, Mat3s& dst) {
...
inline int blurQuantize(Mat& src, Mat& dst, int levels) {
    blur5x5(src, dst);
    int b = 255 / levels;
...
inline int cartoon(Mat& src, Mat& dst, int levels, int magThreshold) {
...
inline int negative(Mat& src, Mat& dst) {
    for (int i = 0; i < src.rows; i++) {
        for (int j = 0; j < src.cols; j++) {
            for (int c = 0; c < 3; c++) {
...
inline int flip(Mat& src, Mat& dst) {
    for (int i = 0; i < src.rows; i++) {
        for (int j = 0; j < src.cols; j++) {
            for (int c = 0; c < 3; c++) {
...
inline int rbg_filter(Mat& src, Mat& dst, string str) {
    for (int i = 0; i < src.rows; i++) {
        for (int j = 0; j < src.cols; j++) {
            if (str == "red") {
                for (int c = 0; c < 2; c++) {
...
inline int sketch(Mat& src, Mat& dst) {
...
inline int color_pop(Mat& src, Mat& dst) {
